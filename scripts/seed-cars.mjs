// One-off seed script: inserts the original demo cars into Supabase.
// Run once after creating the `cars` table (see scripts/schema.sql):
//   node --env-file=.env.local scripts/seed-cars.mjs
// (Node 20.6+ supports --env-file. On older Node, export SUPABASE_URL and
// SUPABASE_SECRET_KEY in the shell before running instead.)
...
const safetyCore = ["ABS", "ESP", "Tempomat", "Parkovací senzory", "Couvací kamera"];
...
function carToRow(car) {
  return {
    ...
    version: car.version ?? "",
    year: car.year,
    registration_date: car.registrationDate || null,
    mileage: car.mileage,
    price: car.price,
    price_without_vat: car.priceWithoutVat ?? null,
    ...
    service_history: car.serviceHistory ?? null,
    stk_valid_until: car.stkValidUntil || null,
...
const { data, error } = await supabase.from("cars").upsert(rows, { onConflict: "id" }).select("slug");
...
console.log(`Seeded ${data.length} cars:`, data.map((c) => c.slug).join(", "));
